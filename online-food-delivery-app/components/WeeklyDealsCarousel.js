...
const WeeklyDealsCarousel = ({ weeklyDeals = [] }) => {
  const [currentIndex, setCurrentIndex] = useState(0)
  const navigation = useNavigation()

  if (!Array.isArray(weeklyDeals) || weeklyDeals.length === 0) return null

  const handlePressDeal = (deal) => {
    navigation.navigate("FilteredPlatos", {
      filterType: deal.filterType,
      title: deal.title,
  ...
  return (
    <View style={styles.wrapper}>
      <Carousel
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => handlePressDeal(item)} activeOpacity={0.85}>
            <LinearGradient
